...
test('M5G3 release coherency versions the composer assets and shell without touching private media cache', () => {
  ...
  assert.equal(build, 'm5g3-production-hotfix-20260923-1');
  assert.equal(version, build);
  assert.match(html, /left-for-you\.css\?v=us-m5g3-1/);
  assert.match(html, /left-for-you\.js\?v=us-m5g3-1/);
  assert.match(worker, /const CACHE_NAME = "us-shell-static-runtime-27"/);
