...
self.addEventListener("install", (event) => {
  event.waitUntil((async () => {
    ...
    // Cache the shell, but never store raw app.js.
    await cache.addAll(APP_SHELL);

    // Critical fix: the very first controlled launch after an update must
    // already receive AUTH_BOOTSTRAP + app + stories, not raw app.js.
    await transformedAppJs(
      new Request(new URL("/app.js", self.location.origin).href, { cache: "no-store" })
    );
